...
// Adicionar documento
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'adicionar_documento') {
    ...
    if (empty($aluno_id) || empty($tipo_documento)) {
        ...
        try {
            $pdo = getDBConnection();
            $stmt = $pdo->prepare("INSERT INTO documentacao_alunos (aluno_id, tipo_documento, numero_documento, data_emissao, data_validade, status, observacoes, criado_por) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$aluno_id, $tipo_documento, $numero_documento, $data_emissao ?: null, $data_validade ?: null, $status, $observacoes, $_SESSION['usuario_id']]);
            
            logAudit('DOCUMENTACAO_CREATE', 'documentacao_alunos', $pdo->lastInsertId(), null, ['aluno_id' => $aluno_id, 'tipo' => $tipo_documento]);
...
// Atualizar documento
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'atualizar_documento') {
    ...
    if (empty($documento_id)) {
        ...
        try {
            $pdo = getDBConnection();
            $stmt = $pdo->prepare("UPDATE documentacao_alunos SET status = ?, data_validade = ?, observacoes = ?, atualizado_por = ? WHERE id = ?");
            $stmt->execute([$status, $data_validade ?: null, $observacoes, $_SESSION['usuario_id'], $documento_id]);
            
            logAudit('DOCUMENTACAO_UPDATE', 'documentacao_alunos', $documento_id, null, ['status' => $status]);
...
if (isset($_GET['action']) && $_GET['action'] === 'excluir' && isset($_GET['id'])) {
    try {
        $pdo = getDBConnection();
        $stmt = $pdo->prepare("DELETE FROM documentacao_alunos WHERE id = ?");
        $stmt->execute([intval($_GET['id'])]);
        
        logAudit('DOCUMENTACAO_DELETE', 'documentacao_alunos', intval($_GET['id']), null, []);
...
try {
    ...
    $stmt = $pdo->query("
        ...
        FROM documentacao_alunos d
        JOIN usuarios u ON d.aluno_id = u.id
        ORDER BY d.data_emissao DESC, u.nome_completo
    ");
    $documentos = $stmt->fetchAll();
} catch (PDOException $e) {
    error_log("Erro ao obter documentação: " . $e->getMessage());
}
...
<head>
    ...
    <title>Controle de Documentação | Portal da Secretaria</title>
...
<body class="bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
                ...
                <div class="flex items-center gap-3">
                    <a href="index.php" class="flex items-center gap-2 sm:gap-3 group">
                        ...
                        <div class="hidden sm:block">
                            ...
                            <span class="block text-amarelo-destaque font-extrabold text-xs sm:text-sm">DOCUMENTAÇÃO</span>
                        </div>
                    </a>
                </div>
    ...
    <main class="px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex items-center justify-between mb-8">
            <h2 class="text-2xl font-bold text-azul-principal">Controle de Documentação</h2>
            <button onclick="document.getElementById('modal-novo-documento').classList.remove('hidden')" class="bg-gradient-to-r from-azul-principal to-verde-complementar text-white px-6 py-3 rounded-xl hover:shadow-lg transition-all font-semibold">
        ...
        <div class="glass-card rounded-3xl shadow-xl overflow-hidden">
            <div class="p-6 border-b border-gray-100 bg-gradient-to-r from-azul-principal to-azul-claro">
                <h3 class="text-xl font-display font-bold text-white">
                    <i class="fas fa-list mr-2"></i>Documentos Cadastrados
                </h3>
            </div>
            <div class="p-6">
                        ...
                        <table class="w-full">
                            <thead>
                                <tr class="text-left text-xs sm:text-sm text-gray-500 border-b border-gray-100 bg-gray-50">
                                    ...
                                    <th class="px-4 sm:px-6 py-4">Validade</th>
                            ...
                            <tbody>
                                <?php foreach ($documentos as $documento): ?>
                                    <tr class="border-b border-gray-50 hover:bg-gray-50">
                                        <td class="px-4 sm:px-6 py-4 font-medium text-gray-800"><?php echo htmlspecialchars($documento['aluno_nome']); ?></td>
                                        <td class="px-4 sm:px-6 py-4 text-gray-600"><?php echo htmlspecialchars($tipos_documento[$documento['tipo_documento']] ?? $documento['tipo_documento']); ?></td>
                                        <td class="px-4 sm:px-6 py-4 text-gray-600"><?php echo htmlspecialchars($documento['numero_documento'] ?? '-'); ?></td>
                                        <td class="px-4 sm:px-6 py-4 text-gray-600"><?php echo $documento['data_emissao'] ? date('d/m/Y', strtotime($documento['data_emissao'])) : '-'; ?></td>
                                        <td class="px-4 sm:px-6 py-4 text-gray-600"><?php echo $documento['data_validade'] ? date('d/m/Y', strtotime($documento['data_validade'])) : '-'; ?></td>
                                        <td class="px-4 sm:px-6 py-4">
                                            <span class="px-2 py-1 rounded-full text-xs font-semibold 
                                                <?php 
                                                $cor_status = match($documento['status']) {
                                        ...
                                        <td class="px-4 sm:px-6 py-4">
                                            <div class="flex gap-2">
                                                <button onclick="editarDocumento(<?php echo $documento['id']; ?>)" class="text-azul-principal hover:text-azul-claro transition-colors">
                                                    <i class="fas fa-edit"></i>
                                                </button>
                                                <a href="?action=excluir&id=<?php echo $documento['id']; ?>" class="text-red-600 hover:text-red-700 transition-colors" onclick="return confirm('Deseja realmente excluir este documento?')">
    ...
    <div id="modal-novo-documento" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
        <div class="glass-card rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div class="p-6 border-b border-gray-100 bg-gradient-to-r from-azul-principal to-azul-claro flex items-center justify-between">
                <h3 class="text-xl font-display font-bold text-white">Novo Documento</h3>
                <button onclick="document.getElementById('modal-novo-documento').classList.add('hidden')" class="text-white hover:text-white/80 transition-colors">
                    <i class="fas fa-times text-xl"></i>
                </button>
            </div>
            <form method="POST" action="" class="p-6">
                <input type="hidden" name="action" value="adicionar_documento">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <select name="tipo_documento" required class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-azul-principal focus:border-transparent">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Número do Documento</label>
                        <input type="text" name="numero_documento" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-azul-principal focus:border-transparent">
                    </div>
                    
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Data Emissão</label>
                        <input type="date" name="data_emissao" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-azul-principal focus:border-transparent">
                    </div>
                </div>
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Data Validade</label>
                        <input type="date" name="data_validade" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-azul-principal focus:border-transparent">
                    </div>
                ...
                <div class="flex justify-end gap-3">
                    <button type="button" onclick="document.getElementById('modal-novo-documento').classList.add('hidden')" class="px-6 py-3 border border-gray-300 rounded-xl hover:bg-gray-50 transition-colors">
                        Cancelar
                    </button>
