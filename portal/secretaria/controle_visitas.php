...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'registrar_visita') {
    $nome_visitante = sanitizeInput($_POST['nome_visitante'] ?? '');
    $tipo_visita = sanitizeInput($_POST['tipo_visita'] ?? '');
    ...
    if (empty($nome_visitante) || empty($tipo_visita) || empty($horario)) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'atualizar_status') {
    ...
    $status = sanitizeInput($_POST['status'] ?? 'agendada');
    ...
    if (empty($visita_id)) {
        ...
        try {
            ...
            $success = 'Status da visita atualizado com sucesso!';
        } catch (PDOException $e) {
...
if (isset($_GET['action']) && $_GET['action'] === 'excluir' && isset($_GET['id'])) {
    try {
        ...
        logAudit('VISITA_DELETE', 'visitas', intval($_GET['id']), null, []);
        
        header('Location: controle_visitas.php');
        exit();
    } catch (PDOException $e) {
        error_log("Erro ao excluir visita: " . $e->getMessage());
...
<head>
    ...
    <title>Controle de Visitas | Portal da Secretaria</title>
...
<body class="bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
                ...
                <div class="flex items-center gap-3">
                    <a href="index.php" class="flex items-center gap-2 sm:gap-3 group">
                        ...
                        <div class="hidden sm:block">
                            ...
                            <span class="block text-amarelo-destaque font-extrabold text-xs sm:text-sm">VISITAS</span>
                        </div>
                    </a>
                </div>
    ...
    <main class="px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mb-8">
            <div class="glass-card rounded-3xl p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="text-right">
                        <p class="text-gray-500 text-sm">Agendadas</p>
                        <p class="text-3xl font-bold text-yellow-500"><?php echo $estatisticas['total_agendadas'] ?? 0; ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="glass-card rounded-3xl p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="text-right">
                        <p class="text-gray-500 text-sm">Realizadas</p>
                        <p class="text-3xl font-bold text-verde-complementar"><?php echo $estatisticas['total_realizadas'] ?? 0; ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="glass-card rounded-3xl p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="text-right">
                        ...
                        <p class="text-3xl font-bold text-red-500"><?php echo $estatisticas['total_canceladas'] ?? 0; ?></p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-between mb-8">
            <h2 class="text-2xl font-bold text-azul-principal">Controle de Visitas</h2>
            <button onclick="document.getElementById('modal-nova-visita').classList.remove('hidden')" class="bg-gradient-to-r from-azul-principal to-verde-complementar text-white px-6 py-3 rounded-xl hover:shadow-lg transition-all font-semibold">
        ...
        <div class="glass-card rounded-3xl shadow-xl overflow-hidden">
            <div class="p-6 border-b border-gray-100 bg-gradient-to-r from-azul-principal to-azul-claro">
                <h3 class="text-xl font-display font-bold text-white">
                    <i class="fas fa-users mr-2"></i>Visitas Registradas
                </h3>
            </div>
            <div class="p-6">
                                    ...
                                    <tr class="border-b border-gray-50 hover:bg-gray-50">
                                        ...
                                        <td class="px-4 sm:px-6 py-4">
                                            <span class="px-2 py-1 rounded-full text-xs font-semibold 
                                                ...
                                                echo $cor_status;
                                                ?>">
                                                <?php echo ucfirst($status_visita[$visita['status']] ?? $visita['status']); ?>
                                            </span>
                                        </td>
                                        <td class="px-4 sm:px-6 py-4">
                                            <div class="flex gap-2">
                                                <button onclick="atualizarStatus(<?php echo $visita['id']; ?>)" class="text-azul-principal hover:text-azul-claro transition-colors">
                                                    <i class="fas fa-edit"></i>
                                                </button>
                                                <a href="?action=excluir&id=<?php echo $visita['id']; ?>" class="text-red-600 hover:text-red-700 transition-colors" onclick="return confirm('Deseja realmente excluir esta visita?')">
    ...
    <!-- Modal Nova Visita -->
    <div id="modal-nova-visita" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
        <div class="glass-card rounded-3xl shadow-2xl w-full max-w-md">
            <div class="p-6 border-b border-gray-100 bg-gradient-to-r from-azul-principal to-verde-complementar flex items-center justify-between">
                <h3 class="text-xl font-display font-bold text-white">Nova Visita</h3>
                <button onclick="document.getElementById('modal-nova-visita').classList.add('hidden')" class="text-white hover:text-white/80 transition-colors">
            ...
            <form method="POST" action="" class="p-6">
                ...
                <div class="mb-4">
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Nome do Visitante</label>
                    <input type="text" name="nome_visitante" required class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-azul-principal focus:border-transparent">
                </div>
                
                <div class="mb-4">
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Tipo de Visita</label>
                    <select name="tipo_visita" required class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-azul-principal focus:border-transparent">
                        <?php foreach ($tipos_visita as $key => $label): ?>
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="date" name="data_visita" value="<?php echo date('Y-m-d'); ?>" required class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-azul-principal focus:border-transparent">
                    </div>
                ...
                <div class="flex justify-end gap-3">
                    <button type="button" onclick="document.getElementById('modal-nova-visita').classList.add('hidden')" class="px-6 py-3 border border-gray-300 rounded-xl hover:bg-gray-50 transition-colors">
                        Cancelar
                    </button>
