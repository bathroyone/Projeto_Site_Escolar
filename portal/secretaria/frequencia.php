...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'registrar_frequencia') {
    $aluno_id = intval($_POST['aluno_id'] ?? 0);
    $data = sanitizeInput($_POST['data'] ?? date('Y-m-d'));
    $status = sanitizeInput($_POST['status'] ?? 'presente');
    $observacoes = sanitizeInput($_POST['observacoes'] ?? '');
    
    if (empty($aluno_id) || empty($data)) {
        ...
        try {
            ...
            if ($stmt->fetch()) {
                $error = 'Já existe registro de frequência para este aluno nesta data.';
            } else {
                $stmt = $pdo->prepare("INSERT INTO frequencia (aluno_id, data, status, observacoes, criado_por) VALUES (?, ?, ?, ?, ?)");
                $stmt->execute([$aluno_id, $data, $status, $observacoes, $_SESSION['usuario_id']]);
                
                logAudit('FREQUENCIA_CREATE', 'frequencia', $pdo->lastInsertId(), null, ['aluno_id' => $aluno_id, 'data' => $data, 'status' => $status]);
                
                $success = 'Frequência registrada com sucesso!';
            }
        } catch (PDOException $e) {
            error_log("Erro ao registrar frequência: " . $e->getMessage());
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'atualizar_frequencia') {
    $frequencia_id = intval($_POST['frequencia_id'] ?? 0);
    $status = sanitizeInput($_POST['status'] ?? 'presente');
    $observacoes = sanitizeInput($_POST['observacoes'] ?? '');
    
    if (empty($frequencia_id)) {
        $error = 'ID da frequência é obrigatório.';
    } else {
        try {
            $pdo = getDBConnection();
            $stmt = $pdo->prepare("UPDATE frequencia SET status = ?, observacoes = ?, atualizado_por = ? WHERE id = ?");
            $stmt->execute([$status, $observacoes, $_SESSION['usuario_id'], $frequencia_id]);
            
            logAudit('FREQUENCIA_UPDATE', 'frequencia', $frequencia_id, null, ['status' => $status]);
            
            $success = 'Frequência atualizada com sucesso!';
        } catch (PDOException $e) {
            error_log("Erro ao atualizar frequência: " . $e->getMessage());
...
if (isset($_GET['action']) && $_GET['action'] === 'excluir' && isset($_GET['id'])) {
    try {
        ...
        logAudit('FREQUENCIA_DELETE', 'frequencia', intval($_GET['id']), null, []);
        
        header('Location: frequencia.php');
        exit();
    } catch (PDOException $e) {
        error_log("Erro ao excluir frequência: " . $e->getMessage());
        $error = 'Erro ao excluir frequência.';
    }
}
...
// Obter registros de frequência
$frequencia_registros = [];
try {
    $pdo = getDBConnection();
    $stmt = $pdo->query("
        SELECT f.*, u.nome_completo as aluno_nome, t.nome as turma_nome
        FROM frequencia f
        JOIN usuarios u ON f.aluno_id = u.id
        LEFT JOIN matriculas m ON u.id = m.aluno_id AND m.status = 'ativa'
        LEFT JOIN turmas t ON m.turma_id = t.id
        ORDER BY f.data DESC, u.nome_completo
    ");
    $frequencia_registros = $stmt->fetchAll();
} catch (PDOException $e) {
    error_log("Erro ao obter registros de frequência: " . $e->getMessage());
}
...
try {
    ...
    $stmt = $pdo->query("
        SELECT 
            COUNT(CASE WHEN status = 'presente' THEN 1 END) as total_presente,
            COUNT(CASE WHEN status = 'falta' THEN 1 END) as total_falta,
            COUNT(CASE WHEN status = 'falta_justificada' THEN 1 END) as total_falta_justificada,
            COUNT(CASE WHEN status = 'atraso' THEN 1 END) as total_atraso
        FROM frequencia
        WHERE data >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)
    ");
    $estatisticas_frequencia = $stmt->fetch();
} catch (PDOException $e) {
...
<head>
    ...
    <title>Controle de Frequência | Portal da Secretaria</title>
...
<body class="bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
                ...
                <div class="flex items-center gap-3">
                    <a href="index.php" class="flex items-center gap-2 sm:gap-3 group">
                        ...
                        <div class="hidden sm:block">
                            ...
                            <span class="block text-amarelo-destaque font-extrabold text-xs sm:text-sm">FREQUÊNCIA</span>
                        </div>
                    </a>
                </div>
    ...
    <main class="px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 sm:gap-6 mb-8">
            <div class="glass-card rounded-3xl p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="text-right">
                        ...
                        <p class="text-3xl font-bold text-verde-complementar"><?php echo $estatisticas_frequencia['total_presente'] ?? 0; ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="glass-card rounded-3xl p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="text-right">
                        ...
                        <p class="text-3xl font-bold text-red-500"><?php echo $estatisticas_frequencia['total_falta'] ?? 0; ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="glass-card rounded-3xl p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="text-right">
                        <p class="text-gray-500 text-sm">Faltas Justificadas</p>
                        <p class="text-3xl font-bold text-yellow-500"><?php echo $estatisticas_frequencia['total_falta_justificada'] ?? 0; ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="glass-card rounded-3xl p-6">
                <div class="flex items-center justify-between mb-4">
                    <div class="w-12 h-12 bg-gradient-to-br from-orange-500 to-orange-400 rounded-xl flex items-center justify-center shadow-lg">
                        <i class="fas fa-clock text-white text-xl"></i>
                    </div>
                    <div class="text-right">
                        <p class="text-gray-500 text-sm">Atrasos</p>
                        <p class="text-3xl font-bold text-orange-500"><?php echo $estatisticas_frequencia['total_atraso'] ?? 0; ?></p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-between mb-8">
            <h2 class="text-2xl font-bold text-azul-principal">Controle de Frequência</h2>
            <button onclick="document.getElementById('modal-nova-frequencia').classList.remove('hidden')" class="bg-gradient-to-r from-azul-principal to-verde-complementar text-white px-6 py-3 rounded-xl hover:shadow-lg transition-all font-semibold">
                <i class="fas fa-plus mr-2"></i>Registrar Frequência
            </button>
        </div>
        ...
        <div class="glass-card rounded-3xl shadow-xl overflow-hidden">
            <div class="p-6 border-b border-gray-100 bg-gradient-to-r from-azul-principal to-azul-claro">
                <h3 class="text-xl font-display font-bold text-white">
                    <i class="fas fa-list mr-2"></i>Registros de Frequência
                </h3>
            </div>
            <div class="p-6">
                <?php if (count($frequencia_registros) > 0): ?>
                    <div class="overflow-x-auto">
                        <table class="w-full">
                            ...
                            <tbody>
                                <?php foreach ($frequencia_registros as $registro): ?>
                                    <tr class="border-b border-gray-50 hover:bg-gray-50">
                                        <td class="px-4 sm:px-6 py-4 font-medium text-gray-800"><?php echo htmlspecialchars($registro['aluno_nome']); ?></td>
                                        <td class="px-4 sm:px-6 py-4 text-gray-600"><?php echo htmlspecialchars($registro['turma_nome'] ?? '-'); ?></td>
                                        <td class="px-4 sm:px-6 py-4 text-gray-600"><?php echo date('d/m/Y', strtotime($registro['data'])); ?></td>
                                        <td class="px-4 sm:px-6 py-4">
                                            <span class="px-2 py-1 rounded-full text-xs font-semibold 
                                                ...
                                                $cor_status = match($registro['status']) {
                                                    ...
                                                    'falta_justificada' => 'bg-yellow-100 text-yellow-600',
                                                    'atraso' => 'bg-orange-100 text-orange-600',
                                                    default => 'bg-gray-100 text-gray-600'
                                                };
                                                echo $cor_status;
                                                ?>">
                                                <?php echo $status_frequencia[$registro['status']] ?? $registro['status']; ?>
                                            </span>
                                        </td>
                                        <td class="px-4 sm:px-6 py-4 text-gray-600"><?php echo htmlspecialchars(substr($registro['observacoes'] ?? '-', 0, 30)); ?></td>
                                        <td class="px-4 sm:px-6 py-4">
                                            <div class="flex gap-2">
                                                <button onclick="editarFrequencia(<?php echo $registro['id']; ?>)" class="text-azul-principal hover:text-azul-claro transition-colors">
                ...
                <?php else: ?>
                    <div class="text-center py-8 text-gray-500">
                        ...
                        <p>Nenhum registro de frequência cadastrado.</p>
                    </div>
                <?php endif; ?>
            </div>
        </div>
    ...
    <div id="modal-nova-frequencia" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
        <div class="glass-card rounded-3xl shadow-2xl w-full max-w-md">
            <div class="p-6 border-b border-gray-100 bg-gradient-to-r from-azul-principal to-verde-complementar flex items-center justify-between">
                <h3 class="text-xl font-display font-bold text-white">Registrar Frequência</h3>
                <button onclick="document.getElementById('modal-nova-frequencia').classList.add('hidden')" class="text-white hover:text-white/80 transition-colors">
                    <i class="fas fa-times text-xl"></i>
                </button>
            </div>
            <form method="POST" action="" class="p-6">
                <input type="hidden" name="action" value="registrar_frequencia">
                ...
                <div class="mb-4">
                    ...
                    <input type="date" name="data" value="<?php echo date('Y-m-d'); ?>" required class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-azul-principal focus:border-transparent">
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select name="status" required class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-azul-principal focus:border-transparent">
                        <?php foreach ($status_frequencia as $key => $label): ?>
                ...
                <div class="flex justify-end gap-3">
                    <button type="button" onclick="document.getElementById('modal-nova-frequencia').classList.add('hidden')" class="px-6 py-3 border border-gray-300 rounded-xl hover:bg-gray-50 transition-colors">
                        Cancelar
                    </button>
