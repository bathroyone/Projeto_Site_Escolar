...
// Adicionar autorização/termo
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'adicionar_autorizacao') {
    $aluno_id = intval($_POST['aluno_id'] ?? 0);
    $tipo_autorizacao = sanitizeInput($_POST['tipo_autorizacao'] ?? '');
    ...
    if (empty($aluno_id) || empty($tipo_autorizacao)) {
        $error = 'Por favor, preencha todos os campos obrigatórios.';
    } else {
        try {
            $pdo = getDBConnection();
            $stmt = $pdo->prepare("INSERT INTO autorizacoes_termos (aluno_id, tipo_autorizacao, descricao, data_inicio, data_fim, status, criado_por) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$aluno_id, $tipo_autorizacao, $descricao, $data_inicio, $data_fim ?: null, $status, $_SESSION['usuario_id']]);
            
            logAudit('AUTORIZACAO_CREATE', 'autorizacoes_termos', $pdo->lastInsertId(), null, ['aluno_id' => $aluno_id, 'tipo' => $tipo_autorizacao]);
            
            $success = 'Autorização/termo adicionado com sucesso!';
        } catch (PDOException $e) {
            error_log("Erro ao adicionar autorização: " . $e->getMessage());
            $error = 'Erro ao adicionar autorização.';
        }
    }
}

// Atualizar autorização
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'atualizar_autorizacao') {
    $autorizacao_id = intval($_POST['autorizacao_id'] ?? 0);
    $status = sanitizeInput($_POST['status'] ?? 'pendente');
    $data_fim = sanitizeInput($_POST['data_fim'] ?? '');
    
    if (empty($autorizacao_id)) {
        $error = 'ID da autorização é obrigatório.';
    } else {
        try {
            $pdo = getDBConnection();
            $stmt = $pdo->prepare("UPDATE autorizacoes_termos SET status = ?, data_fim = ?, atualizado_por = ? WHERE id = ?");
            $stmt->execute([$status, $data_fim ?: null, $_SESSION['usuario_id'], $autorizacao_id]);
            
            logAudit('AUTORIZACAO_UPDATE', 'autorizacoes_termos', $autorizacao_id, null, ['status' => $status]);
            
            $success = 'Autorização atualizada com sucesso!';
        } catch (PDOException $e) {
            error_log("Erro ao atualizar autorização: " . $e->getMessage());
...
if (isset($_GET['action']) && $_GET['action'] === 'excluir' && isset($_GET['id'])) {
    try {
        $pdo = getDBConnection();
        $stmt = $pdo->prepare("DELETE FROM autorizacoes_termos WHERE id = ?");
        $stmt->execute([intval($_GET['id'])]);
        
        logAudit('AUTORIZACAO_DELETE', 'autorizacoes_termos', intval($_GET['id']), null, []);
        
        header('Location: autorizacoes.php');
        exit();
    } catch (PDOException $e) {
        error_log("Erro ao excluir autorização: " . $e->getMessage());
...
$tipos_autorizacao = [
    ...
    'saida_com_responsavel' => 'Saída com Responsável',
...
<head>
    ...
    <title>Gestão de Autorizações e Termos | Portal da Secretaria</title>
...
<body class="bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
                ...
                <div class="flex items-center gap-3">
                    <a href="index.php" class="flex items-center gap-2 sm:gap-3 group">
                        ...
                        <div class="hidden sm:block">
                            <span class="text-white font-bold text-xs sm:text-sm tracking-wide">AUTORIZAÇÕES E</span>
                            <span class="block text-amarelo-destaque font-extrabold text-xs sm:text-sm">TERMOS</span>
                        </div>
                    </a>
                </div>
    ...
    <main class="px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex items-center justify-between mb-8">
            <h2 class="text-2xl font-bold text-azul-principal">Gestão de Autorizações e Termos</h2>
            <button onclick="document.getElementById('modal-nova-autorizacao').classList.remove('hidden')" class="bg-gradient-to-r from-azul-principal to-verde-complementar text-white px-6 py-3 rounded-xl hover:shadow-lg transition-all font-semibold">
                <i class="fas fa-plus mr-2"></i>Nova Autorização
            </button>
        </div>
        ...
        <div class="glass-card rounded-3xl shadow-xl overflow-hidden">
            <div class="p-6 border-b border-gray-100 bg-gradient-to-r from-azul-principal to-azul-claro">
                <h3 class="text-xl font-display font-bold text-white">
                    <i class="fas fa-file-signature mr-2"></i>Autorizações e Termos
                </h3>
            </div>
            <div class="p-6">
                <?php if (count($autorizacoes) > 0): ?>
                    <div class="overflow-x-auto">
                        <table class="w-full">
                            ...
                            <tbody>
                                <?php foreach ($autorizacoes as $autorizacao): ?>
                                    <tr class="border-b border-gray-50 hover:bg-gray-50">
                                        <td class="px-4 sm:px-6 py-4 font-medium text-gray-800"><?php echo htmlspecialchars($autorizacao['aluno_nome']); ?></td>
                                        <td class="px-4 sm:px-6 py-4 text-gray-600"><?php echo htmlspecialchars($tipos_autorizacao[$autorizacao['tipo_autorizacao']] ?? $autorizacao['tipo_autorizacao']); ?></td>
                                        <td class="px-4 sm:px-6 py-4 text-gray-600"><?php echo htmlspecialchars(substr($autorizacao['descricao'] ?? '-', 0, 40)); ?></td>
                                        <td class="px-4 sm:px-6 py-4 text-gray-600">
                                            <?php echo date('d/m/Y', strtotime($autorizacao['data_inicio'])); ?>
                                            <?php if ($autorizacao['data_fim']): ?>
                                                a <?php echo date('d/m/Y', strtotime($autorizacao['data_fim'])); ?>
                                            <?php endif; ?>
                                        </td>
                                        <td class="px-4 sm:px-6 py-4">
                                            <span class="px-2 py-1 rounded-full text-xs font-semibold 
                                                <?php 
                                                $cor_status = match($autorizacao['status']) {
                                                ...
                                                echo $cor_status;
                                                ?>">
                                                <?php echo ucfirst($autorizacao['status']); ?>
                                            </span>
                                        </td>
                                        <td class="px-4 sm:px-6 py-4">
                                            <div class="flex gap-2">
                                                <button onclick="editarAutorizacao(<?php echo $autorizacao['id']; ?>)" class="text-azul-principal hover:text-azul-claro transition-colors">
                                                    <i class="fas fa-edit"></i>
                                                </button>
                                                <a href="?action=excluir&id=<?php echo $autorizacao['id']; ?>" class="text-red-600 hover:text-red-700 transition-colors" onclick="return confirm('Deseja realmente excluir esta autorização?')">
                ...
                <?php else: ?>
                    <div class="text-center py-8 text-gray-500">
                        ...
                        <p>Nenhuma autorização/termo cadastrado.</p>
                    </div>
                <?php endif; ?>
            </div>
        </div>
    ...
    <div id="modal-nova-autorizacao" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
        <div class="glass-card rounded-3xl shadow-2xl w-full max-w-md">
            <div class="p-6 border-b border-gray-100 bg-gradient-to-r from-azul-principal to-verde-complementar flex items-center justify-between">
                <h3 class="text-xl font-display font-bold text-white">Nova Autorização/Termo</h3>
                <button onclick="document.getElementById('modal-nova-autorizacao').classList.add('hidden')" class="text-white hover:text-white/80 transition-colors">
                    <i class="fas fa-times text-xl"></i>
                </button>
            </div>
            <form method="POST" action="" class="p-6">
                <input type="hidden" name="action" value="adicionar_autorizacao">
                ...
                <div class="mb-4">
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Tipo de Autorização</label>
                    <select name="tipo_autorizacao" required class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-azul-principal focus:border-transparent">
                        <?php foreach ($tipos_autorizacao as $key => $label): ?>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="descricao" rows="3" required class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-azul-principal focus:border-transparent"></textarea>
                </div>
                ...
                <div class="flex justify-end gap-3">
                    <button type="button" onclick="document.getElementById('modal-nova-autorizacao').classList.add('hidden')" class="px-6 py-3 border border-gray-300 rounded-xl hover:bg-gray-50 transition-colors">
                        Cancelar
                    </button>
