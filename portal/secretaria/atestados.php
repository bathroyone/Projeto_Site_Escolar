...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'emitir_documento') {
    ...
    if (empty($aluno_id) || empty($tipo_documento)) {
        ...
        try {
            $pdo = getDBConnection();
            $stmt = $pdo->prepare("INSERT INTO atestados_certificados (aluno_id, tipo_documento, data_emissao, observacoes, criado_por) VALUES (?, ?, ?, ?, ?)");
            $stmt->execute([$aluno_id, $tipo_documento, $data_emissao, $observacoes, $_SESSION['usuario_id']]);
            
            logAudit('ATESTADO_CREATE', 'atestados_certificados', $pdo->lastInsertId(), null, ['aluno_id' => $aluno_id, 'tipo' => $tipo_documento]);
            
            $success = 'Documento emitido com sucesso!';
        } catch (PDOException $e) {
            error_log("Erro ao emitir documento: " . $e->getMessage());
...
$tipos_documento = [
    ...
    'atestado_conclusao' => 'Certificado de Conclusão',
    'historico_escolar' => 'Histórico Escolar',
    'declaracao_vaga' => 'Declaração de Vaga',
...
<head>
    ...
    <title>Gestão de Atestados e Certificados | Portal da Secretaria</title>
...
<body class="bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
                ...
                <div class="flex items-center gap-3">
                    <a href="index.php" class="flex items-center gap-2 sm:gap-3 group">
                        ...
                        <div class="hidden sm:block">
                            <span class="text-white font-bold text-xs sm:text-sm tracking-wide">ATESTADOS E</span>
                            <span class="block text-amarelo-destaque font-extrabold text-xs sm:text-sm">CERTIFICADOS</span>
                        </div>
                    </a>
                </div>
    ...
    <main class="px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex items-center justify-between mb-8">
            <h2 class="text-2xl font-bold text-azul-principal">Gestão de Atestados e Certificados</h2>
            <button onclick="document.getElementById('modal-novo-documento').classList.remove('hidden')" class="bg-gradient-to-r from-azul-principal to-verde-complementar text-white px-6 py-3 rounded-xl hover:shadow-lg transition-all font-semibold">
                <i class="fas fa-plus mr-2"></i>Emitir Documento
            </button>
        </div>
        ...
        <div class="glass-card rounded-3xl shadow-xl overflow-hidden">
            <div class="p-6 border-b border-gray-100 bg-gradient-to-r from-azul-principal to-azul-claro">
                <h3 class="text-xl font-display font-bold text-white">
                    <i class="fas fa-list mr-2"></i>Documentos Emitidos
                </h3>
            </div>
            <div class="p-6">
                            ...
                            <tbody>
                                <?php foreach ($documentos as $documento): ?>
                                    <tr class="border-b border-gray-50 hover:bg-gray-50">
                                        ...
                                        <td class="px-4 sm:px-6 py-4 text-gray-600"><?php echo date('d/m/Y', strtotime($documento['data_emissao'])); ?></td>
                                        <td class="px-4 sm:px-6 py-4 text-gray-600"><?php echo htmlspecialchars(substr($documento['observacoes'] ?? '-', 0, 50)); ?></td>
                                        <td class="px-4 sm:px-6 py-4">
                                            <div class="flex gap-2">
                                                <button onclick="imprimirDocumento(<?php echo $documento['id']; ?>)" class="text-azul-principal hover:text-azul-claro transition-colors">
                                                    <i class="fas fa-print"></i>
                                                </button>
                                                <button onclick="visualizarDocumento(<?php echo $documento['id']; ?>)" class="text-verde-complementar hover:text-verde-claro transition-colors">
    ...
    <div id="modal-novo-documento" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
        <div class="glass-card rounded-3xl shadow-2xl w-full max-w-md">
            <div class="p-6 border-b border-gray-100 bg-gradient-to-r from-azul-principal to-verde-complementar flex items-center justify-between">
                <h3 class="text-xl font-display font-bold text-white">Emitir Documento</h3>
            ...
            <form method="POST" action="" class="p-6">
                <input type="hidden" name="action" value="emitir_documento">
                ...
                <div class="mb-6">
                    ...
                    <textarea name="observacoes" rows="3" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-azul-principal focus:border-transparent" placeholder="Informações adicionais para o documento..."></textarea>
                </div>
