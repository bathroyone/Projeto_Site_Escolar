...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'adicionar_evento') {
    ...
    if (empty($aluno_id) || empty($tipo_evento) || empty($data_evento)) {
        $error = 'Por favor, preencha todos os campos obrigatórios.';
    } else {
        try {
            $pdo = getDBConnection();
            $stmt = $pdo->prepare("INSERT INTO calendario_pagamentos (aluno_id, tipo_evento, data_evento, valor, descricao, criado_por) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->execute([$aluno_id, $tipo_evento, $data_evento, $valor, $descricao, $_SESSION['usuario_id']]);
            
            logAudit('CALENDARIO_CREATE', 'calendario_pagamentos', $pdo->lastInsertId(), null, ['aluno_id' => $aluno_id, 'tipo' => $tipo_evento]);
...
if (isset($_GET['action']) && $_GET['action'] === 'excluir' && isset($_GET['id'])) {
    try {
        $pdo = getDBConnection();
        $stmt = $pdo->prepare("DELETE FROM calendario_pagamentos WHERE id = ?");
        $stmt->execute([intval($_GET['id'])]);
        
        logAudit('CALENDARIO_DELETE', 'calendario_pagamentos', intval($_GET['id']), null, []);
        
        header('Location: calendario_pagamentos.php');
        exit();
    } catch (PDOException $e) {
...
try {
    ...
    $stmt = $pdo->query("
        ...
        FROM calendario_pagamentos c
        JOIN usuarios u ON c.aluno_id = u.id
        WHERE c.data_evento >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)
...
try {
    ...
    $stmt = $pdo->query("
        ...
        FROM calendario_pagamentos
        WHERE data_evento >= DATE_SUB(CURDATE(), INTERVAL 6 MONTH)
        GROUP BY MONTH(data_evento), YEAR(data_evento)
        ORDER BY ano DESC, mes DESC
    ");
    $eventos_por_mes = $stmt->fetchAll();
} catch (PDOException $e) {
    error_log("Erro ao obter eventos por mês: " . $e->getMessage());
}
...
<head>
    ...
    <title>Calendário de Pagamentos | Portal da Secretaria</title>
...
<body class="bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
                ...
                <div class="flex items-center gap-3">
                    <a href="index.php" class="flex items-center gap-2 sm:gap-3 group">
                        ...
                        <div class="hidden sm:block">
                            <span class="text-white font-bold text-xs sm:text-sm tracking-wide">CALENDÁRIO DE</span>
                            <span class="block text-amarelo-destaque font-extrabold text-xs sm:text-sm">PAGAMENTOS</span>
                        </div>
                    </a>
                </div>
    ...
    <main class="px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex items-center justify-between mb-8">
            <h2 class="text-2xl font-bold text-azul-principal">Calendário de Pagamentos</h2>
            <button onclick="document.getElementById('modal-novo-evento').classList.remove('hidden')" class="bg-gradient-to-r from-azul-principal to-verde-complementar text-white px-6 py-3 rounded-xl hover:shadow-lg transition-all font-semibold">
        ...
        <div class="glass-card rounded-3xl shadow-xl overflow-hidden mb-8">
            <div class="p-6 border-b border-gray-100 bg-gradient-to-r from-azul-principal to-azul-claro">
                <h3 class="text-xl font-display font-bold text-white">
                    <i class="fas fa-calendar-alt mr-2"></i>Resumo por Mês
                </h3>
            </div>
            <div class="p-6">
                <?php if (count($eventos_por_mes) > 0): ?>
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        <?php foreach ($eventos_por_mes as $mes): ?>
                            <?php 
                            $meses_ano = ['Janeiro', 'Fevereiro', 'Março', 'Abril', 'Maio', 'Junho', 'Julho', 'Agosto', 'Setembro', 'Outubro', 'Novembro', 'Dezembro'];
                            $nome_mes = $meses_ano[$mes['mes'] - 1];
                            ?>
                            <div class="bg-gray-50 rounded-xl p-4">
                                <div class="flex items-center justify-between mb-2">
                                    <span class="font-semibold text-gray-800"><?php echo $nome_mes; ?>/<?php echo $mes['ano']; ?></span>
                                    <span class="text-sm text-gray-500"><?php echo $mes['total_eventos']; ?> eventos</span>
                                </div>
                                <div class="text-2xl font-bold text-azul-principal">R$ <?php echo number_format($mes['total_valor'], 2, ',', '.'); ?></div>
                            </div>
                        <?php endforeach; ?>
                    </div>
                <?php else: ?>
        ...
        <div class="glass-card rounded-3xl shadow-xl overflow-hidden">
            <div class="p-6 border-b border-gray-100 bg-gradient-to-r from-amarelo-destaque to-amarelo-claro">
                <h3 class="text-xl font-display font-bold text-azul-escuro">
                    <i class="fas fa-list mr-2"></i>Eventos do Calendário
                </h3>
            </div>
            <div class="p-6">
                        ...
                        <table class="w-full">
                            <thead>
                                <tr class="text-left text-xs sm:text-sm text-gray-500 border-b border-gray-100 bg-gray-50">
                                    ...
                                    <th class="px-4 sm:px-6 py-4">Tipo Evento</th>
                            ...
                            <tbody>
                                <?php foreach ($eventos as $evento): ?>
                                    <tr class="border-b border-gray-50 hover:bg-gray-50">
                                        <td class="px-4 sm:px-6 py-4 font-medium text-gray-800"><?php echo htmlspecialchars($evento['aluno_nome']); ?></td>
                                        <td class="px-4 sm:px-6 py-4 text-gray-600"><?php echo htmlspecialchars($tipos_evento[$evento['tipo_evento']] ?? $evento['tipo_evento']); ?></td>
                                        <td class="px-4 sm:px-6 py-4 text-gray-600"><?php echo date('d/m/Y', strtotime($evento['data_evento'])); ?></td>
                                        <td class="px-4 sm:px-6 py-4 text-gray-600"><?php echo $evento['valor'] > 0 ? 'R$ ' . number_format($evento['valor'], 2, ',', '.') : '-'; ?></td>
                                        <td class="px-4 sm:px-6 py-4 text-gray-600"><?php echo htmlspecialchars(substr($evento['descricao'] ?? '-', 0, 40)); ?></td>
                                        <td class="px-4 sm:px-6 py-4">
                                            <a href="?action=excluir&id=<?php echo $evento['id']; ?>" class="text-red-600 hover:text-red-700 transition-colors" onclick="return confirm('Deseja realmente excluir este evento?')">
                ...
                <?php else: ?>
                    <div class="text-center py-8 text-gray-500">
                        <i class="fas fa-calendar-alt text-4xl mb-4"></i>
                        <p>Nenhum evento cadastrado no calendário.</p>
                    </div>
                <?php endif; ?>
            </div>
        </div>
    ...
    <!-- Modal Novo Evento -->
    <div id="modal-novo-evento" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
        <div class="glass-card rounded-3xl shadow-2xl w-full max-w-md">
            <div class="p-6 border-b border-gray-100 bg-gradient-to-r from-azul-principal to-verde-complementar flex items-center justify-between">
                ...
                <button onclick="document.getElementById('modal-novo-evento').classList.add('hidden')" class="text-white hover:text-white/80 transition-colors">
            ...
            <form method="POST" action="" class="p-6">
                ...
                <div class="mb-4">
                    ...
                    <select name="tipo_evento" required class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-azul-principal focus:border-transparent">
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="date" name="data_evento" required class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-azul-principal focus:border-transparent">
                    </div>
                    
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Valor (opcional)</label>
                        <input type="number" name="valor" step="0.01" min="0" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-azul-principal focus:border-transparent">
                    </div>
                </div>
                ...
                <div class="flex justify-end gap-3">
                    <button type="button" onclick="document.getElementById('modal-novo-evento').classList.add('hidden')" class="px-6 py-3 border border-gray-300 rounded-xl hover:bg-gray-50 transition-colors">
                        Cancelar
                    </button>
