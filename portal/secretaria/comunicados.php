...
// Enviar comunicado
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'enviar_comunicado') {
    $titulo = sanitizeInput($_POST['titulo'] ?? '');
    $mensagem = sanitizeInput($_POST['mensagem'] ?? '');
    $tipo_destinatario = sanitizeInput($_POST['tipo_destinatario'] ?? 'todos');
    ...
    if (empty($titulo) || empty($mensagem)) {
        ...
        try {
            $pdo = getDBConnection();
            $stmt = $pdo->prepare("INSERT INTO comunicados (titulo, mensagem, tipo_destinatario, aluno_id, prioridade, status, criado_por) VALUES (?, ?, ?, ?, ?, 'enviado', ?)");
            $stmt->execute([$titulo, $mensagem, $tipo_destinatario, $aluno_id ?: null, $prioridade, $_SESSION['usuario_id']]);
            
            logAudit('COMUNICADO_CREATE', 'comunicados', $pdo->lastInsertId(), null, ['titulo' => $titulo, 'tipo' => $tipo_destinatario]);
            
            $success = 'Comunicado enviado com sucesso!';
        } catch (PDOException $e) {
            error_log("Erro ao enviar comunicado: " . $e->getMessage());
...
try {
    $pdo = getDBConnection();
    $stmt = $pdo->query("
        SELECT c.*, u.nome_completo as aluno_nome
        FROM comunicados c
...
try {
    ...
    $stmt = $pdo->query("
        SELECT 
            COUNT(*) as total,
            COUNT(CASE WHEN prioridade = 'alta' THEN 1 END) as total_alta,
            COUNT(CASE WHEN prioridade = 'urgente' THEN 1 END) as total_urgente
        FROM comunicados
...
<head>
    ...
    <title>Gestão de Comunicados | Portal da Secretaria</title>
...
<body class="bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen">
                ...
                <div class="flex items-center gap-3">
                    <a href="index.php" class="flex items-center gap-2 sm:gap-3 group">
                        ...
                        <div class="hidden sm:block">
                            ...
                            <span class="block text-amarelo-destaque font-extrabold text-xs sm:text-sm">COMUNICADOS</span>
                        </div>
                    </a>
                </div>
    ...
    <main class="px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mb-8">
            <div class="glass-card rounded-3xl p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="text-right">
                        <p class="text-gray-500 text-sm">Total (30 dias)</p>
                        <p class="text-3xl font-bold text-azul-principal"><?php echo $estatisticas['total'] ?? 0; ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="glass-card rounded-3xl p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="text-right">
                        <p class="text-gray-500 text-sm">Alta Prioridade</p>
                        <p class="text-3xl font-bold text-orange-500"><?php echo $estatisticas['total_alta'] ?? 0; ?></p>
                    </div>
                </div>
            </div>
            ...
            <div class="glass-card rounded-3xl p-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    <div class="text-right">
                        ...
                        <p class="text-3xl font-bold text-red-500"><?php echo $estatisticas['total_urgente'] ?? 0; ?></p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-between mb-8">
            <h2 class="text-2xl font-bold text-azul-principal">Gestão de Comunicados</h2>
            <button onclick="document.getElementById('modal-novo-comunicado').classList.remove('hidden')" class="bg-gradient-to-r from-azul-principal to-verde-complementar text-white px-6 py-3 rounded-xl hover:shadow-lg transition-all font-semibold">
        ...
        <div class="glass-card rounded-3xl shadow-xl overflow-hidden">
            <div class="p-6 border-b border-gray-100 bg-gradient-to-r from-azul-principal to-azul-claro">
                <h3 class="text-xl font-display font-bold text-white">
                    <i class="fas fa-envelope mr-2"></i>Histórico de Comunicados
                </h3>
            </div>
            <div class="p-6">
                        ...
                        <table class="w-full">
                            <thead>
                                <tr class="text-left text-xs sm:text-sm text-gray-500 border-b border-gray-100 bg-gray-50">
                                    ...
                                    <th class="px-4 sm:px-6 py-4">Destinatário</th>
                                    <th class="px-4 sm:px-6 py-4">Aluno</th>
                                    <th class="px-4 sm:px-6 py-4">Prioridade</th>
                            ...
                            <tbody>
                                <?php foreach ($comunicados as $comunicado): ?>
                                    <tr class="border-b border-gray-50 hover:bg-gray-50">
                                        ...
                                        <td class="px-4 sm:px-6 py-4 text-gray-600"><?php echo htmlspecialchars($tipos_destinatario[$comunicado['tipo_destinatario']] ?? $comunicado['tipo_destinatario']); ?></td>
                                        <td class="px-4 sm:px-6 py-4 text-gray-600"><?php echo htmlspecialchars($comunicado['aluno_nome'] ?? '-'); ?></td>
                                        <td class="px-4 sm:px-6 py-4">
                                            <span class="px-2 py-1 rounded-full text-xs font-semibold 
                                                ...
                                                echo $cor_prioridade;
                                                ?>">
                                                <?php echo ucfirst($prioridades[$comunicado['prioridade']] ?? $comunicado['prioridade']); ?>
                                            </span>
                                        </td>
                                        <td class="px-4 sm:px-6 py-4 text-gray-600"><?php echo date('d/m/Y H:i', strtotime($comunicado['created_at'])); ?></td>
                                        <td class="px-4 sm:px-6 py-4">
                                            <span class="px-2 py-1 rounded-full text-xs font-semibold bg-green-100 text-green-600">
                                                <?php echo ucfirst($comunicado['status']); ?>
                                            </span>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
    ...
    <div id="modal-novo-comunicado" class="hidden fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
        <div class="glass-card rounded-3xl shadow-2xl w-full max-w-lg">
            <div class="p-6 border-b border-gray-100 bg-gradient-to-r from-azul-principal to-verde-complementar flex items-center justify-between">
                <h3 class="text-xl font-display font-bold text-white">Novo Comunicado</h3>
                <button onclick="document.getElementById('modal-novo-comunicado').classList.add('hidden')" class="text-white hover:text-white/80 transition-colors">
            ...
            <form method="POST" action="" class="p-6">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="titulo" required class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-azul-principal focus:border-transparent">
                </div>
                
                <div class="mb-4">
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Mensagem</label>
                    <textarea name="mensagem" rows="5" required class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-azul-principal focus:border-transparent"></textarea>
                </div>
                
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Destinatário</label>
                        <select name="tipo_destinatario" required class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-azul-principal focus:border-transparent" onchange="toggleAlunoSelect(this.value)">
                            <?php foreach ($tipos_destinatario as $key => $label): ?>
                    ...
                    <div>
                        ...
                        <select name="prioridade" required class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-azul-principal focus:border-transparent">
                ...
                <div id="aluno-select-container" class="mb-6 hidden">
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Aluno</label>
                    <select name="aluno_id" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-azul-principal focus:border-transparent">
                ...
                <div class="flex justify-end gap-3">
                    <button type="button" onclick="document.getElementById('modal-novo-comunicado').classList.add('hidden')" class="px-6 py-3 border border-gray-300 rounded-xl hover:bg-gray-50 transition-colors">
                        Cancelar
                    </button>
    ...
    <script>
        ...
        function toggleAlunoSelect(value) {
            const container = document.getElementById('aluno-select-container');
            if (value === 'aluno_especifico') {
