...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'criar_recurso') {
    ...
    if (empty($turma_id) || empty($titulo)) {
        ...
        try {
            $pdo = getDBConnection();
            $stmt = $pdo->prepare("
                INSERT INTO recursos_compartilhados (professor_id, turma_id, titulo, descricao, link, tipo, data_criacao) 
                VALUES (?, ?, ?, ?, ?, ?, NOW())
            ");
            $stmt->execute([$professor_id, $turma_id, $titulo, $descricao, $link, $tipo]);
            
            $success = 'Recurso compartilhado com sucesso!';
        } catch (PDOException $e) {
            error_log("Erro ao criar recurso: " . $e->getMessage());
...
try {
    $pdo = getDBConnection();
    $stmt = $pdo->prepare("
        SELECT r.*, t.nome as turma_nome, t.serie 
        FROM recursos_compartilhados r 
...
<div class="mb-6">
    <div class="flex items-center justify-between">
        <h2 class="text-xl font-semibold text-gray-800">Compartilhamento de Recursos</h2>
...
<div class="bg-white rounded-xl border border-gray-200 shadow-sm">
    <div class="p-4 border-b border-gray-200">
        <h3 class="font-semibold text-gray-800">Recursos Compartilhados</h3>
    </div>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                <?php foreach ($recursos as $recurso): ?>
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 font-medium text-gray-800"><?php echo htmlspecialchars($recurso['titulo']); ?></td>
                        <td class="px-6 py-4 text-gray-600"><?php echo htmlspecialchars($recurso['tipo'] ?? '-'); ?></td>
                        <td class="px-6 py-4 text-gray-600"><?php echo htmlspecialchars($recurso['turma_nome'] . ' - ' . $recurso['serie']); ?></td>
                        <td class="px-6 py-4 text-sm">
                            <?php if ($recurso['link']): ?>
                                <a href="<?php echo htmlspecialchars($recurso['link']); ?>" target="_blank" class="text-primary-600 hover:text-primary-800 mr-2">
                                    <i class="fas fa-external-link-alt"></i>
                                </a>
                            <?php endif; ?>
                            <a href="?action=excluir&id=<?php echo $recurso['id']; ?>" class="text-red-600 hover:text-red-800" onclick="return confirm('Tem certeza que deseja excluir este recurso?');">
    ...
    <?php if (empty($recursos)): ?>
        <div class="p-8 text-center text-gray-500">
            ...
            <p>Nenhum recurso compartilhado.</p>
        </div>
    <?php endif; ?>
</div>
    ...
    <div class="absolute inset-0 flex items-center justify-center p-4">
        <div class="bg-white rounded-xl shadow-xl max-w-2xl w-full">
            <div class="p-6 border-b border-gray-200 flex items-center justify-between">
                <h3 class="text-lg font-semibold text-gray-800">Novo Recurso Compartilhado</h3>
            ...
            <form method="POST" action="" class="p-6">
                ...
                <div class="mb-4">
                    ...
                    <input type="url" name="link" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500">
                </div>
                ...
                <button type="submit" class="w-full bg-primary-600 text-white font-medium py-2 rounded-lg hover:bg-primary-700 transition-colors">
                    <i class="fas fa-save mr-2"></i>Compartilhar Recurso
                </button>
            </form>
        </div>
    </div>
