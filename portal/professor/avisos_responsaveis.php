...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'criar_aviso') {
    ...
    $tipo = sanitizeInput($_POST['tipo'] ?? 'outro');
    $titulo = sanitizeInput($_POST['titulo'] ?? '');
    $mensagem = sanitizeInput($_POST['mensagem'] ?? '');
    
    if (empty($aluno_id) || empty($titulo) || empty($mensagem)) {
        $error = 'Por favor, preencha todos os campos obrigatórios.';
    } else {
        try {
            $pdo = getDBConnection();
            $stmt = $pdo->prepare("INSERT INTO avisos_responsaveis (professor_id, aluno_id, turma_id, tipo, titulo, mensagem, criado_por) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$_SESSION['usuario_id'], $aluno_id, $turma_id ?: null, $tipo, $titulo, $mensagem, $_SESSION['usuario_id']]);
            
            logAudit('AVISO_RESPONSAVEL_CREATE', 'avisos_responsaveis', $pdo->lastInsertId(), null, ['aluno_id' => $aluno_id, 'tipo' => $tipo]);
...
// Enviar aviso
if (isset($_GET['action']) && $_GET['action'] === 'enviar' && isset($_GET['id'])) {
    try {
        $pdo = getDBConnection();
        $stmt = $pdo->prepare("UPDATE avisos_responsaveis SET status = 'enviado', data_envio = NOW() WHERE id = ? AND professor_id = ?");
        $stmt->execute([intval($_GET['id']), $_SESSION['usuario_id']]);
        header('Location: avisos_responsaveis.php');
        exit();
    } catch (PDOException $e) {
...
if (isset($_GET['action']) && $_GET['action'] === 'excluir' && isset($_GET['id'])) {
    try {
        $pdo = getDBConnection();
        $stmt = $pdo->prepare("DELETE FROM avisos_responsaveis WHERE id = ? AND professor_id = ?");
        $stmt->execute([intval($_GET['id']), $_SESSION['usuario_id']]);
        header('Location: avisos_responsaveis.php');
        exit();
    } catch (PDOException $e) {
...
try {
    $pdo = getDBConnection();
    $stmt = $pdo->query("
        SELECT ar.*, u.nome_completo as aluno_nome, t.nome as turma_nome 
        FROM avisos_responsaveis ar 
        JOIN usuarios u ON ar.aluno_id = u.id 
        LEFT JOIN turmas t ON ar.turma_id = t.id 
...
<head>
    ...
    <title>Avisos aos Responsáveis | Portal do Professor</title>
...
<body class="bg-gray-100 min-h-screen">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="flex items-center justify-between mb-8">
            <div>
                <h1 class="text-3xl font-display font-bold text-azul-principal">Avisos aos Responsáveis</h1>
                <p class="text-gray-600 mt-2">Comunicação com responsáveis dos alunos</p>
            </div>
                ...
                <table class="w-full">
                    <thead>
                        <tr class="text-left text-xs sm:text-sm text-gray-500 border-b border-gray-100 bg-gray-50">
                            ...
                            <th class="px-4 sm:px-6 py-4 hidden md:table-cell">Tipo</th>
                    ...
                    <tbody>
                        <?php foreach ($avisos as $aviso): ?>
                            <tr class="border-b border-gray-50 hover:bg-gray-50">
                                <td class="px-4 sm:px-6 py-4 font-medium text-gray-800"><?php echo htmlspecialchars($aviso['titulo']); ?></td>
                                <td class="px-4 sm:px-6 py-4 text-gray-600 text-sm"><?php echo htmlspecialchars($aviso['aluno_nome']); ?></td>
                                <td class="px-4 sm:px-6 py-4 hidden md:table-cell">
                                    <span class="px-2 py-1 rounded-full text-xs font-semibold 
                                        <?php 
                                        $cor_tipo = match($aviso['tipo']) {
                                            'elogio' => 'bg-green-100 text-green-600',
                                            'alerta' => 'bg-red-100 text-red-600',
                                            'comportamento' => 'bg-yellow-100 text-yellow-600',
                                            'academico' => 'bg-blue-100 text-blue-600',
                                            'frequencia' => 'bg-purple-100 text-purple-600',
                                ...
                                <td class="px-4 sm:px-6 py-4 text-gray-600 text-sm hidden md:table-cell"><?php echo htmlspecialchars($aviso['turma_nome'] ?? '-'); ?></td>
                                <td class="px-4 sm:px-6 py-4">
                                    <span class="px-2 py-1 rounded-full text-xs font-semibold 
                                        <?php 
                                        $cor_status = match($aviso['status']) {
                                            'pendente' => 'bg-yellow-100 text-yellow-600',
                                            'enviado' => 'bg-blue-100 text-blue-600',
                                            'lido' => 'bg-green-100 text-green-600',
                                            default => 'bg-gray-100 text-gray-600'
                                        };
                                        echo $cor_status;
                                        ?>">
                                ...
                                <td class="px-4 sm:px-6 py-4">
                                    <div class="flex gap-1">
                                        <?php if ($aviso['status'] === 'pendente'): ?>
                                            <a href="?action=enviar&id=<?php echo $aviso['id']; ?>" class="p-2 rounded-lg hover:bg-blue-100 text-blue-600 transition-colors" title="Enviar">
                                                <i class="fas fa-paper-plane"></i>
                                            </a>
                                        <?php endif; ?>
                                        <a href="?action=excluir&id=<?php echo $aviso['id']; ?>" class="p-2 rounded-lg hover:bg-red-100 text-red-600 transition-colors" onclick="return confirm('Tem certeza que deseja excluir?');">
        ...
        <div class="absolute inset-0 flex items-center justify-center p-4">
            <div class="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
                <div class="p-6 border-b border-gray-100 flex items-center justify-between">
                    <h2 class="text-xl font-display font-bold text-azul-principal">Novo Aviso</h2>
                ...
                <form method="POST" action="" class="p-6">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="mensagem" name="mensagem" rows="4" required
                            class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-azul-principal focus:border-transparent"
                            placeholder="Mensagem para o responsável"></textarea>
                    </div>
