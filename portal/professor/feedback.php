...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'criar_feedback') {
    ...
    $tipo = sanitizeInput($_POST['tipo'] ?? 'orientacao');
    $titulo = sanitizeInput($_POST['titulo'] ?? '');
    $conteudo = sanitizeInput($_POST['conteudo'] ?? '');
    $data_feedback = sanitizeInput($_POST['data_feedback'] ?? date('Y-m-d'));
    
    if (empty($aluno_id) || empty($titulo) || empty($conteudo)) {
        $error = 'Por favor, preencha todos os campos obrigatórios.';
    } else {
        try {
            $pdo = getDBConnection();
            $stmt = $pdo->prepare("INSERT INTO feedback_alunos (professor_id, aluno_id, turma_id, disciplina, tipo, titulo, conteudo, data_feedback, criado_por) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$_SESSION['usuario_id'], $aluno_id, $turma_id ?: null, $disciplina, $tipo, $titulo, $conteudo, $data_feedback, $_SESSION['usuario_id']]);
            
            logAudit('FEEDBACK_CREATE', 'feedback_alunos', $pdo->lastInsertId(), null, ['aluno_id' => $aluno_id, 'tipo' => $tipo]);
...
if (isset($_GET['action']) && $_GET['action'] === 'enviar' && isset($_GET['id'])) {
    try {
        $pdo = getDBConnection();
        $stmt = $pdo->prepare("UPDATE feedback_alunos SET status = 'enviado' WHERE id = ?");
...
if (isset($_GET['action']) && $_GET['action'] === 'excluir' && isset($_GET['id'])) {
    try {
        $pdo = getDBConnection();
        $stmt = $pdo->prepare("DELETE FROM feedback_alunos WHERE id = ? AND professor_id = ?");
...
try {
    ...
    $stmt = $pdo->query("
        ...
        FROM feedback_alunos fa 
        JOIN usuarios u ON fa.aluno_id = u.id 
        LEFT JOIN turmas t ON fa.turma_id = t.id 
        WHERE fa.professor_id = ? 
...
<head>
    ...
    <title>Feedback para Alunos | Portal do Professor</title>
...
<body class="bg-gray-100 min-h-screen">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="flex items-center justify-between mb-8">
            <div>
                <h1 class="text-3xl font-display font-bold text-azul-principal">Feedback para Alunos</h1>
                <p class="text-gray-600 mt-2">Envie feedbacks construtivos aos alunos</p>
            </div>
                    ...
                    <tbody>
                        <?php foreach ($feedbacks as $feedback): ?>
                            <tr class="border-b border-gray-50 hover:bg-gray-50">
                                <td class="px-4 sm:px-6 py-4 font-medium text-gray-800"><?php echo htmlspecialchars($feedback['titulo']); ?></td>
                                <td class="px-4 sm:px-6 py-4 text-gray-600 text-sm"><?php echo htmlspecialchars($feedback['aluno_nome']); ?></td>
                                <td class="px-4 sm:px-6 py-4">
                                    <span class="px-2 py-1 rounded-full text-xs font-semibold 
                                        <?php 
                                        $cor_tipo = match($feedback['tipo']) {
                                            'elogio' => 'bg-green-100 text-green-600',
                                            'melhoria' => 'bg-orange-100 text-orange-600',
                                            'orientacao' => 'bg-blue-100 text-blue-600',
                                            'avaliacao' => 'bg-purple-100 text-purple-600',
                                            default => 'bg-gray-100 text-gray-600'
                                        };
                                        echo $cor_tipo;
                                        ?>">
                                        <?php echo ucfirst($feedback['tipo']); ?>
                                    </span>
                                </td>
                                <td class="px-4 sm:px-6 py-4 text-gray-600 text-sm hidden md:table-cell"><?php echo htmlspecialchars($feedback['disciplina'] ?? '-'); ?></td>
                                <td class="px-4 sm:px-6 py-4 text-gray-600 text-sm hidden md:table-cell"><?php echo date('d/m/Y', strtotime($feedback['data_feedback'])); ?></td>
                                <td class="px-4 sm:px-6 py-4">
                                    <span class="px-2 py-1 rounded-full text-xs font-semibold 
                                        <?php 
                                        $cor_status = match($feedback['status']) {
                                ...
                                <td class="px-4 sm:px-6 py-4">
                                    <div class="flex gap-1">
                                        <?php if ($feedback['status'] === 'pendente'): ?>
                                            <a href="?action=enviar&id=<?php echo $feedback['id']; ?>" class="p-2 rounded-lg hover:bg-blue-100 text-blue-600 transition-colors" title="Enviar">
                                                <i class="fas fa-paper-plane"></i>
                                            </a>
                                        <?php endif; ?>
                                        <a href="?action=excluir&id=<?php echo $feedback['id']; ?>" class="p-2 rounded-lg hover:bg-red-100 text-red-600 transition-colors" onclick="return confirm('Tem certeza que deseja excluir?');">
        ...
        <div class="absolute inset-0 flex items-center justify-center p-4">
            <div class="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
                <div class="p-6 border-b border-gray-100 flex items-center justify-between">
                    <h2 class="text-xl font-display font-bold text-azul-principal">Novo Feedback</h2>
                ...
                <form method="POST" action="" class="p-6">
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        ...
                        <div>
                            <label for="data_feedback" class="block text-sm font-semibold text-gray-700 mb-2">Data *</label>
                            <input type="date" id="data_feedback" name="data_feedback" required value="<?php echo date('Y-m-d'); ?>"
                                class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-azul-principal focus:border-transparent">
                        </div>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="conteudo" name="conteudo" rows="4" required
                            class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-azul-principal focus:border-transparent"
                            placeholder="Conteúdo do feedback"></textarea>
                    </div>
