...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'criar_escala') {
    ...
    $tipo = sanitizeInput($_POST['tipo'] ?? 'semanal');
    ...
    if (empty($nome) || empty($tipo)) {
        ...
        try {
            $pdo = getDBConnection();
            $stmt = $pdo->prepare("INSERT INTO escalas (nome, tipo, descricao, data_inicio, data_fim, criado_por) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->execute([$nome, $tipo, $descricao, $data_inicio, $data_fim, $_SESSION['usuario_id']]);
            
            logAudit('ESCALA_CREATE', 'escalas', $pdo->lastInsertId(), null, ['nome' => $nome, 'tipo' => $tipo]);
...
// Adicionar horário
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'adicionar_horario') {
    $escala_id = intval($_POST['escala_id'] ?? 0);
    $usuario_id = intval($_POST['usuario_id'] ?? 0);
    $dia_semana = sanitizeInput($_POST['dia_semana'] ?? 'segunda');
    ...
    if (empty($escala_id) || empty($usuario_id) || empty($hora_inicio) || empty($hora_fim)) {
        $error = 'Por favor, preencha todos os campos obrigatórios.';
    } else {
        try {
            $pdo = getDBConnection();
            $stmt = $pdo->prepare("INSERT INTO horarios (escala_id, usuario_id, dia_semana, hora_inicio, hora_fim, atividade, observacoes) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([$escala_id, $usuario_id, $dia_semana, $hora_inicio, $hora_fim, $atividade, $observacoes]);
            
            logAudit('HORARIO_CREATE', 'horarios', $pdo->lastInsertId(), null, ['escala_id' => $escala_id, 'usuario_id' => $usuario_id]);
...
// Excluir escala
if (isset($_GET['action']) && $_GET['action'] === 'excluir_escala' && isset($_GET['id'])) {
    try {
        $pdo = getDBConnection();
...
try {
    $pdo = getDBConnection();
    $stmt = $pdo->query("SELECT * FROM escalas ORDER BY created_at DESC");
...
try {
    $pdo = getDBConnection();
    $stmt = $pdo->query("
        SELECT h.*, e.nome as escala_nome, u.nome_completo as usuario_nome 
        FROM horarios h 
        JOIN escalas e ON h.escala_id = e.id 
...
<head>
    ...
    <title>Horários e Escalas | Portal de Gestão Escolar</title>
...
<body class="bg-gray-100 min-h-screen">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="flex items-center justify-between mb-8">
            <div>
                <h1 class="text-3xl font-display font-bold text-azul-principal">Horários e Escalas</h1>
                <p class="text-gray-600 mt-2">Gestão de escalas de trabalho</p>
            </div>
        ...
        <div class="flex gap-2 mb-6 border-b border-gray-200">
            <button onclick="showTab('escalas')" id="tab-escalas" class="px-6 py-3 font-semibold text-azul-principal border-b-2 border-azul-principal">Escalas</button>
            <button onclick="showTab('horarios')" id="tab-horarios" class="px-6 py-3 font-semibold text-gray-500 hover:text-azul-principal">Horários</button>
        </div>

        <!-- Tab Escalas -->
        <div id="content-escalas" class="tab-content">
                        ...
                        <tbody>
                            <?php foreach ($escalas as $escala): ?>
                                <tr class="border-b border-gray-50 hover:bg-gray-50">
                                    <td class="px-4 sm:px-6 py-4 font-medium text-gray-800"><?php echo htmlspecialchars($escala['nome']); ?></td>
                                    <td class="px-4 sm:px-6 py-4">
                                        <span class="px-2 py-1 rounded-full text-xs font-semibold bg-blue-100 text-blue-600">
                                            <?php echo ucfirst($escala['tipo']); ?>
                                        </span>
                                    </td>
                                    <td class="px-4 sm:px-6 py-4 text-gray-600 text-sm hidden md:table-cell">
                                        <?php echo $escala['data_inicio'] ? date('d/m/Y', strtotime($escala['data_inicio'])) : '-'; ?>
                                        <?php echo $escala['data_fim'] ? ' a ' . date('d/m/Y', strtotime($escala['data_fim'])) : ''; ?>
                                    </td>
                                    <td class="px-4 sm:px-6 py-4">
                                        <span class="px-2 py-1 rounded-full text-xs font-semibold <?php echo $escala['ativa'] ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-600'; ?>">
                                            <?php echo $escala['ativa'] ? 'Ativa' : 'Inativa'; ?>
                                        </span>
                                    </td>
                                    <td class="px-4 sm:px-6 py-4">
                                        <a href="?action=excluir_escala&id=<?php echo $escala['id']; ?>" class="p-2 rounded-lg hover:bg-red-100 text-red-600 transition-colors" onclick="return confirm('Tem certeza que deseja excluir esta escala?');">
        ...
        <div id="content-horarios" class="tab-content hidden">
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-6">
                <h3 class="text-lg font-bold text-azul-principal mb-4">Novo Horário</h3>
                <form method="POST" action="" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <input type="hidden" name="action" value="adicionar_horario">
                    
                    <div>
                        <label for="escala_id" class="block text-sm font-semibold text-gray-700 mb-2">Escala</label>
                        <select id="escala_id" name="escala_id" required
                            ...
                            <?php foreach ($escalas as $escala): ?>
                                <option value="<?php echo $escala['id']; ?>"><?php echo htmlspecialchars($escala['nome']); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    ...
                    <div class="md:col-span-3 grid grid-cols-2 gap-4">
                        <div>
                            <label for="atividade" class="block text-sm font-semibold text-gray-700 mb-2">Atividade</label>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="p-6 border-b border-gray-100">
                    <h3 class="text-lg font-bold text-azul-principal">Horários Cadastrados</h3>
                </div>
                <div class="overflow-x-auto">
                    <table class="w-full">
                        <thead>
                            <tr class="text-left text-xs sm:text-sm text-gray-500 border-b border-gray-100 bg-gray-50">
                                <th class="px-4 sm:px-6 py-4">Escala</th>
                                <th class="px-4 sm:px-6 py-4">Usuário</th>
                                <th class="px-4 sm:px-6 py-4">Dia</th>
                                <th class="px-4 sm:px-6 py-4">Horário</th>
                                <th class="px-4 sm:px-6 py-4 hidden md:table-cell">Atividade</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php foreach ($horarios as $horario): ?>
                                <tr class="border-b border-gray-50 hover:bg-gray-50">
                                    <td class="px-4 sm:px-6 py-4 font-medium text-gray-800"><?php echo htmlspecialchars($horario['escala_nome']); ?></td>
                                    <td class="px-4 sm:px-6 py-4 text-gray-600 text-sm"><?php echo htmlspecialchars($horario['usuario_nome']); ?></td>
                                    <td class="px-4 sm:px-6 py-4">
                                        <span class="px-2 py-1 rounded-full text-xs font-semibold bg-purple-100 text-purple-600">
                                            <?php echo ucfirst($horario['dia_semana']); ?>
                                        </span>
                                    </td>
                                    <td class="px-4 sm:px-6 py-4 text-gray-600 text-sm"><?php echo $horario['hora_inicio']; ?> - <?php echo $horario['hora_fim']; ?></td>
                                    <td class="px-4 sm:px-6 py-4 text-gray-600 text-sm hidden md:table-cell"><?php echo htmlspecialchars($horario['atividade'] ?? '-'); ?></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
        ...
        <div class="absolute inset-0 flex items-center justify-center p-4">
            <div class="bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
                <div class="p-6 border-b border-gray-100 flex items-center justify-between">
                    <h2 class="text-xl font-display font-bold text-azul-principal">Nova Escala</h2>
                ...
                <form method="POST" action="" class="p-6">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="descricao" name="descricao" rows="3"
                            class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-azul-principal focus:border-transparent"
                            placeholder="Descrição da escala"></textarea>
                    </div>
