    ...
    if (empty($nome) || empty($email) || empty($senha) || empty($tipo_usuario)) {
        ...
        try {
            ...
            if ($stmt->fetch()) {
                ...
                $stmt = $pdo->prepare("INSERT INTO usuarios (nome_completo, email, senha, tipo_usuario, turma, serie, matricula) VALUES (?, ?, ?, ?, ?, ?, ?)");
                $stmt->execute([$nome, $email, $senha_hash, $tipo_usuario, $turma, $serie, $matricula]);
                    ...
                    <tbody>
                        <?php foreach ($usuarios as $usuario): ?>
                            <tr class="border-b border-gray-50 hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 text-gray-600"><?php echo htmlspecialchars($usuario['email']); ?></td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-gray-600"><?php echo htmlspecialchars($usuario['matricula'] ?? '-'); ?></td>
                                <td class="px-6 py-4 text-gray-600"><?php echo date('d/m/Y', strtotime($usuario['data_criacao'])); ?></td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-2">
                                        <a href="?action=toggle_status&id=<?php echo $usuario['id']; ?>" class="p-2 rounded-lg hover:bg-gray-100 text-gray-600" title="<?php echo $usuario['ativo'] ? 'Desativar' : 'Ativar'; ?>">
                                            <i class="fas fa-<?php echo $usuario['ativo'] ? 'ban' : 'check'; ?>"></i>
                                        </a>
                                        <?php if ($usuario['id'] != $_SESSION['usuario_id']): ?>
                                            <a href="?action=excluir&id=<?php echo $usuario['id']; ?>" class="p-2 rounded-lg hover:bg-red-100 text-red-600" title="Excluir" onclick="return confirm('Tem certeza que deseja excluir este usuário?');">
